...
export function PantallaCompletado({ icono = 'check-circle', titulo, descripcion, onVolver, textoVolver = 'Volver' }) {
  ...
  return (
    <div className="text-center py-8">
      ...
      {titulo && <h3 className={`text-lg font-black uppercase tracking-wider mb-2 ${tema.title}`}>{titulo}</h3>}
      {descripcion && <p className={`text-sm mb-6 max-w-md mx-auto ${tema.subtitle}`}>{descripcion}</p>}

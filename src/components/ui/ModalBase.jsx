...
export function ModalBase({ show, onClose, maxWidth = 'max-w-lg', children }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center px-4" onClick={onClose}>
      <div
        className={`relative rounded-2xl w-full ${maxWidth} max-h-[85vh] overflow-y-auto p-8 border ${tema.isDark ? 'bg-[#0e162c] border-slate-700/50' : 'bg-white border-slate-200 shadow-sm'}`}

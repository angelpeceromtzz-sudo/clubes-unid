...
function TarjetaPostulacion({ postulacion, tema }) {
  ...
  return (
    <div className={`rounded-2xl border ${tema.isDark ? 'bg-[#0e162c] border-slate-700/50' : 'bg-white border-slate-200 shadow-sm'} overflow-hidden`}>
      <div className="p-5">
        ...
        <button
          onClick={() => setDetallesAbiertos((p) => !p)}
          className={`mt-3 text-[11px] uppercase tracking-wider font-bold flex items-center gap-1 transition-colors cursor-pointer ${tema.isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-3.5 w-3.5 transition-transform ${detallesAbiertos ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {detallesAbiertos && (
          <div className={`mt-3 rounded-xl p-4 space-y-3 ${tema.isDark ? 'bg-[#18223f]' : 'bg-slate-100'}`}>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              <div>
                <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Carrera</span>
                <p className={tema.isDark ? 'text-slate-300' : 'text-slate-700'}>{postulacion.carrera}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Turno</span>
                <p className={tema.isDark ? 'text-slate-300' : 'text-slate-700'}>{postulacion.turno}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Cuatrimestre</span>
                <p className={tema.isDark ? 'text-slate-300' : 'text-slate-700'}>{postulacion.cuatrimestre}°</p>
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Bloque</span>
                <p className={tema.isDark ? 'text-slate-300' : 'text-slate-700'}>{postulacion.bloque_asignado !== 'E' ? `Bloque ${postulacion.bloque_asignado}` : 'Sin asignar'}</p>
              </div>
            </div>

            <div className="flex items-center gap-2 mt-2">
              <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 shrink-0 ${COLORES_ESTATUS[postulacion.status]?.split(' ')[0] || 'text-slate-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <span className={`text-xs font-medium ${tema.isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                {postulacion.status === 'Pendiente' && 'Esperando revisión del presidente'}
                {postulacion.status === 'En revisión' && 'El presidente está revisando tu solicitud'}
                {postulacion.status === 'Preseleccionado' && '¡Has pasado a la siguiente etapa!'}
                {postulacion.status === 'Convocado' && 'Revisa los detalles de tu convocatoria'}
                {postulacion.status === 'Admitido' && '¡Bienvenido al club!'}
              {postulacion.status === 'Aceptado' && 'Has sido aceptado en el club'}
                {postulacion.status === 'Rechazado' && 'Tu solicitud no fue aceptada'}
              </span>
            </div>

            {postulacion.convocatorias && postulacion.convocatorias.length > 0 && (
              <div className={`border-t pt-3 ${tema.isDark ? 'border-slate-700/50' : 'border-slate-200'}`}>
                <p className={`text-[11px] uppercase tracking-wider font-bold mb-2 ${tema.isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                  Convocatorias
                </p>
                {postulacion.convocatorias.map((conv) => (
                  <div key={conv.id_convocatoria} className={`rounded-lg p-3 mb-2 last:mb-0 ${tema.isDark ? 'bg-[#0e162c]' : 'bg-white border border-slate-200'}`}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {postulaciones.map((p) => (
          <TarjetaPostulacion key={p.id_formulario} postulacion={p} tema={tema} />
        ))}
      </div>
    </div>
  );

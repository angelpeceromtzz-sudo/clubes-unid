      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          {!cargando && horarios.length === 0 && (
            <span className={`text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-full border ${modoOscuro ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' : 'bg-amber-50 text-amber-600 border-amber-200'}`}>
              Próximamente
            </span>
          )}
        </div>
      ...
      {cargando ? (
        ...
        <div className={`rounded-xl border border-dashed p-4 md:p-6 text-center ${modoOscuro ? 'border-slate-700 bg-slate-800/30' : 'border-slate-300 bg-slate-50'}`}>
          <div className={`w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center ${modoOscuro ? 'bg-slate-700/50' : 'bg-slate-200'}`}>
            <Icono nombre="clock" strokeWidth={1.5} className={`h-5 w-5 ${modoOscuro ? 'text-slate-500' : 'text-slate-400'}`} />
          </div>
          <p className={`text-xs ${c.text}`}>Próximamente se mostrarán los horarios de las sesiones del club.</p>
        </div>
      ) : (
